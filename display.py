...
def show_dashboard():
    ...
    with st.sidebar:
        ...
        status_pill = f"""
            <div class="status-pill status-{'active' if is_capturing else 'idle'}">
                <div class="status-dot dot-{'active' if is_capturing else 'idle'}"></div>
                {'Live Monitoring' if is_capturing else 'System Suspended'}
            </div>
        """
        ...
        if not is_capturing:
            if st.button("INITIALIZE CAPTURE", use_container_width=True):
                start_capture_thread()
                st.rerun()
        else:
            if st.button("TERMINATE CAPTURE", use_container_width=True):
                stop_capture()
                st.rerun()
    ...
    if df.empty:
        st.markdown("""
            <div class="glass-card" style="text-align: center; padding: 120px 20px;">
                ...
                <h3 style="opacity: 0.6; font-weight: 400;">No Incoming Traffic</h3>
                <p style="opacity: 0.4; font-size: 0.9rem;">Initialize the capture interface to start packet ingestion.</p>
            </div>
        """, unsafe_allow_html=True)
        if is_capturing:
            time.sleep(2)
